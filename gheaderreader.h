...
class GHeaderReader : public GDataReader<std::vector<std::string>>
{
	friend class Reader_F_headers_Test;

      public:
	explicit GHeaderReader(std::string path)
	    : GDataReader(FILENAME, std::move(path))
	{
      ...
      private:
	constexpr static auto FILENAME = "headers";
	void parse();
};
